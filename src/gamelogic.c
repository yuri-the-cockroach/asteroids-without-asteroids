...
void DebugingKeyHandler(ObjectTracker *tracker) {
    ...
    if (IsKeyPressed('9')) {
        for (unsigned int i = 0; i < MAX_OBJECT_COUNT - 1; i++) {
            CreateAsteroid(tracker,
                           (Vector2){ GetRandomValue(0, SCREEN_WIDTH),
                                      GetRandomValue(0, SCREEN_HEIGHT) },

                           (Vector2){ GetRandomValue(-100, 100),
    ...
    if (IsKeyPressed('0')) {
        unsigned int iterations = tracker->objListLen;
        if (iterations > 1) {
            for (unsigned int i = iterations - 1; i > 0; i--) {
                DeleteTrackedObject(tracker, tracker->objList[i]);
            }
        }
    }
    ...
    if (IsKeyPressed('-')) {
        if (tracker->objList[tracker->objListLen - 1])
