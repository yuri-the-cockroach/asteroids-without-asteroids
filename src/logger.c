...
void Logger(const char *restrict inFile, const char *restrict inFunc, const int onLine,
    ...
    // Pass all the arguments to the vsprinf, which will fill it into the message string.
    // If argptr is 0, then it will just be ignored (hopefully)
    ...
    sprintf(formatedTime, "%.2ld-%.2ld-%.2ld.%ld", (time.tv_sec / 3600 + 3) % 24 , time.tv_sec / 60 % 60, time.tv_sec % 60, time.tv_usec);
    // Actually print to stderr
    fprintf(stderr, "[%s] %s %s->%s:%d %s\n", loglvlToString[loglevel], formatedTime, inFile, inFunc, onLine, messageString);
}
