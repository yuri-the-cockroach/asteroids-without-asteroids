#include "headers/objectlogic.h"
#include <raylib.h>
#include <string.h>

void UpdateObjectPos(ObjectStruct *object) {
    object->position.x = fmod(object->position.x + object->speed.x * GetFrameTime(), (float)GetScreenWidth());
    if (object->position.x < 0) object->position.x = (float)GetScreenWidth();
    object->position.y = fmod(object->position.y + object->speed.y * GetFrameTime(), (float)GetScreenHeight());
    if (object->position.y < 0) object->position.y = (float)GetScreenHeight();
}
...
void TransformObjectMatrix(ShapeStruct * shape, float rotationAngle) {
    for (unsigned int current = 0; current < shape->arrayLength; current++) {
        shape->points[current].x = ( shape->refPoints[current].x * cos(rotationAngle) ) - ( shape->refPoints[current].y * sin(rotationAngle) );
        shape->points[current].y = ( shape->refPoints[current].x * sin(rotationAngle) ) + ( shape->refPoints[current].y * cos(rotationAngle) );
    }
}
...
void DrawObject(ObjectStruct * object) {
    ...
    for (unsigned int curPoint = 1; curPoint < object->shape.arrayLength; curPoint++){
        DrawLineEx (
    ...
    (
        ...
        (Vector2){ // Draw to x/y
            object->shape.points[object->shape.arrayLength - 1].x + object->position.x,
            object->shape.points[object->shape.arrayLength - 1].y + object->position.y
        },
...
ShapeStruct InitShape(const Vector2 *pointArray, unsigned int arrayLength, float sizeMult) {
    ...
    memcpy((void*)toReturn.points, (void*)toReturn.refPoints, sizeof(Vector2) * arrayLength);
    return toReturn;
}

ObjectStruct InitObject(ShapeStruct shape) {
    return (ObjectStruct){
        0,            // acceleration speed
        0,            // Rotation speed
        0,                      // Starter heading
        (Vector2){ 0, 0 },      // Starting position of the object
        (Vector2){ 0, 0 },      // Starting speed of the object
        shape
    };
}
